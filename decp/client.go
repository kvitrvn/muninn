...
// Capabilities declares the filters supported by the awarded-contract dataset.
...
// Search implements muninn.Provider by paginating awarded contracts. The
// structured result reports source totals and truncation.
...
func buildWhere(q muninn.Query) string {
	...
	if !q.PublishedFrom.IsZero() {
		date = append(date, fmt.Sprintf(`datenotification >= "%s"`, q.PublishedFrom.Format("2006-01-02")))
	}
...
func mapRecord(rec map[string]any) muninn.Tender {
	t := muninn.Tender{
		Sources:  []muninn.SourceReference{{Provider: "decp", RawFields: rec}},
...
// numeric string. Returns 0 both when the field is absent and when it is
// present but unparseable — the two cases are indistinguishable from the
// return value alone; the raw value remains available in the corresponding
// SourceReference.RawFields entry.
