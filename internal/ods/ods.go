...
type Client struct {
	...
	// Retry configures the retry/backoff behavior applied to requests when the
	// API responds with 429/5xx. The zero value uses httpx.DefaultRetryPolicy.
...
// Search fetches records in pages of maxPageSize within maxOffsetWindow and
// the optional CandidateLimit. ProviderResult reports totals and truncation.
func (c *Client) Search(ctx context.Context, q muninn.Query) (muninn.ProviderResult, error) {
	where := c.Where(q)

	// Effective upper bound: API window, tightened by q.CandidateLimit.
	hardCap := maxOffsetWindow
	if q.CandidateLimit > 0 && q.CandidateLimit < hardCap {
		hardCap = q.CandidateLimit
	}
	...
	for offset := 0; offset < hardCap; offset += maxPageSize {
		...
		if err != nil {
			return muninn.ProviderResult{
				Items:      tenders,
				Total:      totalCount,
				TotalExact: totalCount > 0,
				Truncated:  len(tenders) < totalCount,
			}, err
		}
	...
	return muninn.ProviderResult{
		...
		Total:      totalCount,
		TotalExact: true,
		Truncated:  totalCount > len(tenders),
	}, nil
}
